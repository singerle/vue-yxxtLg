<template>
    ...
    <topSelect :toptitle="toptitle"></topSelect>
		<vHeader @Jump="Jump" :data="header" v-if="header" class="home-header"></vHeader>
		<div class="home-content">
			<vxslxObj v-for="(item,i) in sylx" :data="item" class="mb10" :key="'xslx'+i" :show="flag" @intopage="intopage" @backindex="backindex"></vxslxObj>
      <template v-if="flag">
        <!-- <hjObj class="mb10" :data="cwjf" :titleD="{name:'财务缴费',func_dq:'getXqjfqk',func_xy:'getXyjfqk'}" v-if="cwjf.length>0"></hjObj> -->
        <hjObj class="mb10" :data="hjblqk" :sylx="sylx" :titleD="{name:'环节办理情况',func_dq:'getXqhjblqk',func_xy:'getXqhjblqk'}" v-if="hjblqk.length>0"></hjObj>
        <hjObj class="mb10" :data="xbbl" :sylx="sylx" :titleD="{name:'新生男女比例',func_dq:'getXqnnbl',func_xy:'getXqnnbl'}" v-if="xbbl.length>0"></hjObj>
        <hjObj class="mb10" :data="wsxx" :sylx="sylx" :titleD="{name:'完善信息',func_dq:'getXqjbxx',func_xy:'getXqjbxx'}" v-if="wsxx.length>0&&toptitle!='研究生'"></hjObj>
        <hjObj class="mb10" :data="bxgm" :sylx="sylx" :titleD="{name:'保险购买',func_dq:'getXqbxgm',func_xy:'getXybxgm'}" v-if="bxgm.length>0&&toptitle!='研究生'"></hjObj>
        <hjObj class="mb10" :data="lstd" :sylx="sylx" :titleD="{name:'绿色通道',func_dq:'getXqlstd',func_xy:'getXylstd'}" v-if="lstd.length>0&&toptitle!='研究生'"></hjObj>
        <hjObj class="mb10" :data="qsbl" :sylx="sylx" :titleD="{name:'寝室办理',func_dq:'getXqssbl',func_xy:'getXyssbl'}" v-if="qsbl.length>0&&toptitle!='研究生'"></hjObj>
      </template>
		</div>
    </div>
</template>
  ...
  created(){
    getXqlb().then((res) => {
      if(res.code == '40001'){
        ...
        this.getHomeInfo(this.xqList[0],this.pageSylx)
      }
    })
  },
  methods: {
    ...
    intopage(val){
      console.log(val)
      this.flag = val.bool
      this.getHomeInfo(this.xqList[0],val.sylx)
      this.toptitle = val.sylx
    },
    backindex(bool){
      if(bool){
        //回退初始化
        ...
        //回到刚进入的生源类别页面
        this.getHomeInfo(this.xqList[0],this.toptitle)
        this.flag = true
      }
    },
    getHomeInfo(xq,lx){
      ...
      this.wsxx = []
      this.hjblqk = []
      getHomeInfo({xq: encodeURI(xq),sylx:encodeURI(lx)}).then((res) => {
        if(res.code == '40001'){
