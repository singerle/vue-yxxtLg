<template>
    <div class="hjObj">
    	<header>
    		...
    		<span class="fr ft-12 my-color-gray" @click="togo(titleD)" v-if="titleD.name=='保险购买'||titleD.name=='绿色通道'||titleD.name=='新生男女比例'||titleD.name=='完善信息'||titleD.name=='环节办理情况'?false:true">更多</span>
    	</header>
    	<div class="hjObj-content">
    		<div class="hjObj-content-chart" v-if="titleD.name!='环节办理情况'">
...
export default {
  ...
  props: ['data','titleD','sylx'],
  methods: {
	...mapActions([
	'sylxset_sylx'
	]),
  	togo(key){console.log(JSON.stringify(this.sylx))
		  console.log(key)
		  var sylx = JSON.parse(JSON.stringify(this.sylx))[0].sylx
		this.sylxset_sylx(JSON.stringify(this.sylx)[0].sylx)
