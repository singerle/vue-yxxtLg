    ...
    <div class="region">
    	...
    	<div v-if="activeIndex == 0" class="regionContent">
    		...
    		<div class="regionContent-table">
                ...
                <td v-if="item.wgm>=0">{{item.wgm}}</td>
    	...
    	<div v-else-if="activeIndex == 1" class="regionContent">
    		...
    		<div class="regionContent-table">
                ...
                <td v-if="item.wgm>=0">{{item.wgm}}</td>
    						<td>{{((item.sd*10000)/(item.zrs*100))!=0? ((item.sd*10000)/(item.zrs*100)).toFixed(2):0}}%<span class="iconfont icon-gengduo" @click="togo(item.xybh)"></span></td>
    					</tr>
    				</table>
    			</div>
    		</div>
...
export default {
  ...
  computed: {
    ...
    pieOption(){
      ...
      // var dataArr = []
      // var colorArr = []
      // var formatStr = '<b>{point.name}</b><br><b style="font-size:20px;">{point.percentage:.1f} %</b><br>{y}人'
      // this.snsw.snrs = 0
      // this.snsw.swrs = 0
      let sn = {
                  name: '省内',
      ...
      if(this.snsw.snrs==this.snsw.swrs&&this.snsw.swrs==0&&this.snsw.snrs==0){
        var dataArr = [defal]
        var colorArr = ['#f2f2f2']
        var formatStr = '<b>{point.name}</b>'
        // console.log(dataArr,colorArr)
      }else{
        var dataArr = [sn,sw]
        var colorArr = ['#60C998', '#80dbf6']
        var formatStr = '<b>{point.name}</b><br><b style="font-size:20px;">{point.percentage:.1f} %</b><br>{y}人'
      }
      let option = {
            title: {
  ...
  methods: {
    ...
    getXqbdqk() {
      if(this.key){
        ...
        console.log(k)
        this.set_dqData({func: k.func_dq,p: {xq: encodeURI(this.get_xq),type: this.types.filter(item => item.active)[0].type,snsw: this.snswType,sylx: encodeURI(this.get_sylx)}}).then((res) => {
