    ...
    return new Promise((resolve) => {
    	getDataApi('yxtj/xqbxgm', params).then((res) => {
    		if(params.type==1){
	    		...
	    		if(res.code == '40001'){
	    			res.content.dqfb.forEach((r,i) => {
	    				d.push({sd: r.yblrs,yd: r.ysqrs,fq: r.yfqrs,sf: r.sf,wgm:r.zrs-r.yblrs-r.ysqrs-r.yfqrs,zrs: r.zrs})
	    			})
	    			snsw.snbdrs = res.content.snsw.snblrs
	    			snsw.snrs = res.content.snsw.snrs
	    			snsw.swbdrs = res.content.snsw.swblrs
	    		...
	    		res.title = ['省份','已购买','待支付','不购买','未办理','百分比']
    		}else if(params.type==2){
    			let d = []
    			res.content.forEach((r, i) => {
    				d.push({xy: r.xy, sd: r.yblrs, yd: r.ysqrs, yfqrs: r.yfqrs,xybh: r.xybh,wgm:r.zrs-r.yblrs-r.ysqrs-r.yfqrs,zrs: r.zrs})
    			})
    			res.content = d
    			res.title = ['学院','已购买','待支付','不购买','未办理','百分比']
    		}
    		resolve(res)
    	})
    })
...
export const getXybxgm = (params) => {
    return new Promise((resolve) => {
    	getDataApi('yxtj/xybxgm', params).then((res) => {
    		...
    		if(res.code == '40001'){
    			res.content.forEach((r, i) => {
    				d.push({sd: r.yblrs,yd: r.ysqrs,yfqrs: r.yfqrs,zy: r.zy,wgm:r.zrs-r.yblrs-r.ysqrs-r.yfqrs,zrs: r.zrs})
    			})
    		}
    		res.content = d
    		let r = Object.assign(res,{title: ['专业','已购买','待支付','不购买','未购买','百分比']})//未购买
    		resolve(r)
    	})
    })
}
    ...
    return new Promise((resolve) => {
    	getDataApi('yxtj/xqlstd', params).then((res) => {
    		if(params.type==1){
	    		...
	    		if(res.code == '40001'){
	    			res.content.dqfb.forEach((r,i) => {
	    				d.push({sd: r.yblrs,yd: r.bblrs,sf: r.sf,zrs:r.zrs,fq:r.zrs-r.bblrs-r.yblrs})
	    			})
	    			snsw.snbdrs = res.content.snsw.snblrs
	    			snsw.snrs = res.content.snsw.snrs
	    			snsw.swbdrs = res.content.snsw.swblrs
	    		...
	    		res.title = ['省份','已申请','不申请','未办理','百分比']
    		}else if(params.type==2){
    			let d = []
    			res.content.forEach((r, i) => {
    				d.push({xy: r.xy, sd: r.yblrs, yd: r.bblrs,xybh: r.xybh,zrs: r.zrs,yfqrs:r.zrs-r.bblrs-r.yblrs})
    			})
    			res.content = d
    			res.title = ['学院','已申请','不申请','未办理','百分比']
    		}
    		resolve(res)
    	})
    })
...
export const getXylstd = (params) => {
    return new Promise((resolve) => {
    	getDataApi('yxtj/xylstd', params).then((res) => {
    		...
    		if(res.code == '40001'){
    			res.content.forEach((r, i) => {
    				d.push({sd: r.yblrs,yd: r.bblrs,zy: r.zy,zrs: r.zrs})
    			})
    		}
    		res.content = d
    		let r = Object.assign(res,{title: ['学院','已申请','不申请','未办理','百分比']})
    		resolve(r)
    	})
    })
}
