...
<!--     	<popup v-show="wancheng">
      ...
      <popup1 class="tc" v-show="wancheng" @event="wancheng=false">
        <div>
          <img src="static/student-index-pictures/lg.png" :style="{'width':'100%'}">
...
<!--     	<ul class="app-list">
		...
		<ul class="app-list" :title=" <p>{{datas}}</p>" v-if="process.length>0" style="margin-top:.11rem">
    		<li v-for="(item, index) in datas" @click="togo(item.pauId, item.state, item.studentJumpPath)" :style="{'background-image':'url('+item.imgUrl+')','background-size': 'cover'}">
...
export default {
  ...
  computed: {
  	showHref(){
  		let obj = []
  		this.process.length>0 && (obj = this.process.filter(item => item.state==1))
