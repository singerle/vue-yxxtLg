<template>
    <div class="newsCentral-wrapper">
      ...
      <p v-if="tableData">暂无数据</p>
    </div>
</template>

<script>
//import { deletenews, sortNumber } from 'oa/api/statis/news'
import {fetchbjsj,fetchbjbdsj} from 'oa/api/dataClass/getData'
  ...
  export default {
    ...
    methods: {
      ...
      modifynews(row) {
        ...
        // zymc: 应用统计学（应用数理统计）
        // xy: 01
         var row = {xy:row.xy,zy:row.zy,bj:row.bj,xymc:encodeURI(row.xymc),zymc:encodeURI(row.zymc)}
        fetchbjsj(row).then(res => {
        let fileName = row.bj+'班新生报道签字确认单.docx'
    ...
    　　window.URL.revokeObjectURL(href); //释放掉blob对象 
      ...
      //导出汇总表
       deletenew(row) {
         var row = {xy:row.xy,zy:row.zy,bj:row.bj,xymc:encodeURI(row.xymc),zymc:encodeURI(row.zymc)}
        fetchbjbdsj(row).then(res => {
          let fileName = row.bj + '班新生报到情况汇总表.docx'
        //这里res.data是返回的blob对象     
          var blob = new Blob([res.data],  { type: 'application/msword' }); 
        //var blob = new Blob([res.data], {type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8'}); //application/vnd.openxmlformats-officedocument.spreadsheetml.sheet这里表示xlsx类型
      　　var downloadElement = document.createElement('a');
