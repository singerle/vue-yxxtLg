...
import {classData} from 'oa/router/index'
    ...
    if (to.path === '/login') {
      ...
      if (store.getters.roles.length === 0) {
        ...
        store.dispatch('FetchUserInfo').then(res => {
          if (!res) {
          ...
          } else {console.log(store.getters.roles+'/'+store.getters.rolesStatis)
            if(store.getters.roles==1){
              // router.options.routes.splice(3,1)
            }
            if(store.getters.roles==2&&store.getters.rolesStatis[0]==true){
              console.log(router.options.routes)
              // var a = router.options.routes.splice(0,1)
              // console.log(a)
            ...
            store.dispatch('GenerateRoutes', {data: store.getters.roles, rolesStatis: store.getters.rolesStatis}).then(() => {
              ...
              router.addRoutes(store.getters.activeRouter.concat(classData)) // 添加动态路由
              next({...to})
            })
          }
        }).catch((error) => {
